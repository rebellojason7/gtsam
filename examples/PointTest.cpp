...
int main(int argc, char* argv[]) {
  ...
  for(size_t i=0; i<3; i++){
    graph.emplace_shared<PriorFactor<Point3> >(Symbol('l', i), points[i], pointNoise); // add directly to graph
  }

  
  noiseModel::Diagonal::shared_ptr odometryNoise = noiseModel::Diagonal::Sigmas(Vector3(0.4, 0.4, 0.4));
  for(size_t i=3; i<8; i++){
    for(size_t j=0; j<3; j++){
      graph.emplace_shared<BetweenFactor<Point3> >(Symbol('l',i), Symbol('l',mapping[i-3][j]), points[mapping[i-3][j]]-points[i], odometryNoise);
    }
  }
